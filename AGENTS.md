...
At the start of a terminal session inside a connected project, read the approved context and check assigned work with `teambrain tasks --root <memory-root> --team <team-id> --project <project-id> --actor <github-login>`. Tell the user about their open assigned tasks, including due dates, before beginning unrelated work. Use the GitHub login/TeamBrain actor identity for matching; never infer a person from an email or display name.

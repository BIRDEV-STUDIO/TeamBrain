...
Document imports are project-scoped. The dashboard accepts text, CSV/TSV/JSON, DOCX, XLSX/XLSM and PDF files up to 15 MB, extracts readable text, and writes only a Markdown representation under `memory/knowledge/imports/`. The original binary is not retained by default. Each import records the original filename, SHA-256, project identity and import time so the Markdown can be reviewed and traced. Automatic Git synchronization includes the generated Markdown; users should review confidential content before uploading. PDF extraction requires the bundled `pypdf` runtime or an installed `pdftotext` tool; legacy `.xls` is intentionally not accepted yet.
